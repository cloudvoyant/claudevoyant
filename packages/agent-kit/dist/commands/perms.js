...
export function taskRunnerAllow(command) {
    if (command.startsWith('just'))
        return 'Bash(just:*)';
    if (command.startsWith('mise'))
        return 'Bash(mise run:*)';
    if (command.startsWith('task'))
        return 'Bash(task:*)';
    if (command.startsWith('make'))
        return 'Bash(make:*)';
    if (command.startsWith('pnpm'))
        return 'Bash(pnpm run:*)';
...
export function detectAgent(override) {
    if (override)
        return override;
...
export function buildClaudeAllow(plugins, cwd) {
    const entries = new Set(BASELINE);
...
export function mergeClaudeAllow(filePath, newEntries) {
    const existing = readJsonFile(filePath);
    const permissions = (existing.permissions ?? {});
    const existingAllow = new Set(permissions.allow ?? []);
...
export function permsCommand() {
    ...
    cmd
        ...
        .action((opts) => {
        const plugins = opts.plugins.split(',').map((p) => p.trim().toLowerCase());
        const agent = detectAgent(opts.agent);
        ...
        if (agent === 'claude-code') {
            ...
            const current = (existing.permission ?? {});
            existing.permission = {
                read: 'allow',
                edit: 'allow',
                write: 'allow',
            ...
            console.log(JSON.stringify({ agent, target, entries: Object.keys(existing.permission).length }, null, 2));
        }
        else if (agent === 'vscode-copilot') {
            console.log(JSON.stringify({
                agent,
                message: 'VS Code Copilot permissions are set per agent via tools: frontmatter. Reinstall with: bash scripts/install-vscode.sh',
            }));
        }
        else {
