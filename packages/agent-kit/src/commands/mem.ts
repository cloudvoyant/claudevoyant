...
function collectMdFiles(dir: string, rootDir: string): string[] {
  ...
  function walk(currentDir: string): void {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (currentDir === rootDir && EXCLUDED_DIRS.includes(entry.name)) continue;
...
export function memCommand(): Command {
  ...
  mem
    ...
    .action((opts) => {
      ...
      for (const filePath of mdFiles) {
        ...
        if (!fm) continue;
        if (!fm.type || !fm.tags || fm.tags.length === 0) continue;
  ...
  mem
    ...
    .action((opts) => {
      ...
      let entries: MemEntry[] = JSON.parse(raw);
