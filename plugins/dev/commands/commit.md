Create a git commit following conventional commit standards with a professional,
concise message.
...
If you need to see detailed changes, read the modified files directly using the
Read tool rather than running `git diff` (which can trigger permission checks on
diff content).
...
- `style`: Code style/formatting (not docs style) → no version bump (appears in
  changelog)
...
- If changes span multiple types (docs + feat), ask user which to prioritize or
  suggest splitting into multiple commits
- Avoid mixing unrelated changes in one commit

**Breaking changes:**

- If breaking change, add exclamation mark after type (example: feat!: redesign
  API)
