...
Read `.claudevoyant/style/config.json` and CLAUDE.md to compile context information:

```bash
# Extract contexts from config
CONFIGURED_CONTEXTS=$(jq -r '.contexts | keys[]' .claudevoyant/style/config.json)
...
Update `.claudevoyant/style/config.json`:
...
Manage global context settings in `.claudevoyant/style/config.json`:
