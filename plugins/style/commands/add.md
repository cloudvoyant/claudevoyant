...
If section doesn't exist, offer to create it:
```
Section "{Section Name}" doesn't exist in CLAUDE.md.
Create new section with context tags @context: {contexts}?
[Yes/No]
```
...
Calculate estimated token count for the section:

```bash
# Rough estimate: 4 characters ≈ 1 token
SECTION_SIZE=$(wc -c < section.txt)
TOKEN_ESTIMATE=$((SECTION_SIZE / 4))
```
