...
2. Infer the type (`recipe` or `styleguide`) and tags from content. If ambiguous:
```
AskUserQuestion:
...
5. Read the configured docs directory:
```bash
...
If no matches found, say so and suggest `/mem2 learn` to capture new knowledge.
