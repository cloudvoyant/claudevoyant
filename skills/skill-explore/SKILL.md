---
description: "Research existing agent skills before building new ones. Searches agentskill.sh and published skill repos for skills close to what you need, reads their SKILL.md files, identifies absorb opportunities and dependencies. Triggers on: \"skill explore\", \"explore skills\", \"find existing skills for\", \"what skills exist for\", \"research skills\"."
name: skill:explore
license: MIT
...
argument-hint: "[topic] [--thorough]"
context: fork
---

> **Compatibility**: If AskUserQuestion is unavailable, present options as numbered list.

## Skill Requirements

```bash
# Check npx skills CLI is available
npx skills --version 2>/dev/null || echo "MISSING: npx skills CLI not available"
```
...
```bash
TOPIC="${1:-}"           # exploration topic (optional)
THOROUGH="${2:-false}"   # --thorough flag
```
